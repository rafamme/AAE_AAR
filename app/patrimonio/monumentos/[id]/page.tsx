...
export default async function MonumentDetailPage({params}:Props){
  ...
  return <main className="wrap catalog-page">
    <nav className="catalog-nav"><Link href="/">Mapa</Link><Link href="/patrimonio">Patrimonio</Link><Link href={`/patrimonio/localidades/${location.id}`}>{location.name}</Link><Link href="/patrimonio/rutas">Rutas</Link><Link href="/area-socios">Mi cuenta</Link></nav>
    <header className="detail-hero"><div className="eyebrow">{[monument.architectural_type,monument.century,location.name].filter(Boolean).join(' · ')}</div><h1>{monument.name}</h1><p>{monument.description??'Monumento incluido en el catálogo patrimonial.'}</p><SavePlaceControls monumentId={monument.id}/></header>
