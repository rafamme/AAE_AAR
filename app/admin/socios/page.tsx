...
export default async function AdminMembersPage({ searchParams }: { searchParams: Promise<{ mensaje?: string; estado?: string }> }) {
  ...
  return <>
    <header className="page-heading"><div><div className="muted">Administración · Socios</div><h1>Socios y altas</h1><p>Alta, baja, modificación, mantenimiento de fichas, estados, permisos y roles.</p></div></header>
    ...
    <nav className="admin-nav">
      ...
      <a href="/admin/socios?estado=inactive">Bajas ({counts.inactive ?? 0})</a>
    </nav>
    ...
    <section className="admin-member-list">
      {(members ?? []).map((member) => {
        ...
        return <article className="admin-member-card" key={member.id}>
          <div className="admin-member-title"><div><div className="catalog-card-meta">{member.member_number ? `Socio nº ${member.member_number}` : member.status === 'pending' ? 'Solicitud pendiente' : 'Sin número'}</div><h2><Link href={`/admin/socios/${member.id}`}>{member.first_name} {member.last_name}</Link></h2><p>{member.email_public || 'Sin email público'} · Estado: <strong>{member.status}</strong></p></div><Link className="button-link secondary" href={`/admin/socios/${member.id}`}>Abrir ficha →</Link></div>
          {member.status === 'pending' && <div className="role-actions">
            <form action={approveMember}><input type="hidden" name="id" value={member.id}/><input type="hidden" name="return_to" value="/admin/socios"/><button type="submit">Aprobar alta y asignar nº</button></form>
            <form action={rejectMember}><input type="hidden" name="id" value={member.id}/><input type="hidden" name="return_to" value="/admin/socios"/><button className="secondary" type="submit">Rechazar solicitud</button></form>
          </div>}
          <form action={updateMember} className="admin-inline-form">
            <input type="hidden" name="id" value={member.id}/><input type="hidden" name="return_to" value="/admin/socios"/>
            <label>Nº socio<input type="number" min="1" name="member_number" defaultValue={member.member_number ?? ''}/></label>
            <label>Estado<select name="status" defaultValue={member.status}><option value="pending">Pendiente</option><option value="active">Activo</option><option value="suspended">Suspendido</option><option value="inactive">Baja / inactivo</option></select></label>
          ...
          <div className="role-actions">
            {availableRoles.map((role) => {
              ...
              return <form action={setMemberRole} key={role}><input type="hidden" name="member_id" value={member.id}/><input type="hidden" name="return_to" value="/admin/socios"/><input type="hidden" name="role" value={role}/><input type="hidden" name="enabled" value={enabled ? 'false' : 'true'}/><button className={enabled ? 'secondary' : ''} type="submit">{enabled ? `Quitar ${role}` : `Dar ${role}`}</button></form>;
            })}
          </div>
        </article>;
      })}
