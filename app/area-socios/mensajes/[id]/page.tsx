...
export default async function MessageThreadPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{mensaje?:string}>}) {
  ...
  const { data: thread } = await supabase.from('message_threads').select('id,subject,is_announcement,created_by').eq('id', id).maybeSingle();
  if (!thread) notFound();
  ...
  await supabase.rpc('mark_thread_read', { target_thread_id: id });
  ...
  const otherName = thread.is_announcement ? 'AAE-AAR' : other ? names.get(other.member_id) ?? 'Socio AAE-AAR' : 'Conversación';

  return <main className="wrap member-area message-thread-page"><header className="page-heading"><div><div className="muted">{thread.is_announcement ? 'AAE-AAR · Comunicado' : 'AAE-AAR · Mensajería privada'}</div><h1>{thread.is_announcement ? thread.subject : otherName}</h1>{!thread.is_announcement && <p>{thread.subject}</p>}</div><Link className="button-link secondary" href="/area-socios/mensajes">Bandeja</Link></header>{mensaje && <p className="notice">{mensaje}</p>}<section className="message-history">{((messages ?? []) as Message[]).map(message => <article className={`message-bubble ${message.sender_id === user.id ? 'mine' : ''}`} key={message.id}><div className="message-meta"><strong>{thread.is_announcement ? 'AAE-AAR' : message.sender_id === user.id ? 'Tú' : otherName}</strong><time>{new Date(message.created_at).toLocaleString('es-ES',{dateStyle:'medium',timeStyle:'short'})}</time></div><p>{message.body}</p>{message.edited_at && <span className="muted">Editado</span>}</article>)}</section>{thread.is_announcement ? <p className="notice">Este comunicado es informativo y no admite respuestas.</p> : <form action={sendReply} className="message-reply-form"><input type="hidden" name="thread_id" value={id}/><label>Responder<textarea name="body" rows={5} required placeholder="Escribe una respuesta…" /></label><button type="submit">Enviar respuesta</button></form>}</main>;
}
